  ...
  <method id="foo"/>
   <resource_type id="rtype">
    ...
    scenario ("The original WADL contains an extension attribute") {
      ...
      assert (normWADL, "//wadl:resource[@path='to' and not(@rax:invisible)]")
      assert (normWADL, "//wadl:resource[@path='my' and not(@rax:invisible)]")
      assert (normWADL, "//wadl:resource[@path='resource' and @rax:invisible='true']")
      assert (normWADL, "empty(//wadl:resource[@path='to']/rax:foo)")
      assert (normWADL, "empty(//wadl:resource[@path='my']/rax:foo)")
    ...
    scenario ("The original WADL contains an extension element in the rax: namespace with an href attribute") {
      ...
      assert (normWADL, "count(//rax:log) = 1")
